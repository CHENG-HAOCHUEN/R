...
gender <- factor("男","男","女")
paul.family <- list(name="Paul", wife="Iris",no.kids=3, kid.age=c(25,28,30), gender,Y.dataframe)
paul.family
paul.family$kid.age
paul.family[4]
paul.family[4][2]#錯誤的
...
x
str(x)
x <- read.table("x.csv",header = TRUE)
x
...
str(iris)
summary(iris)
save(iris,file = "iris.DRATA")
load("iris.DRATA", .GlobalEnv)
#第三章
a <- c(1,2,3)
x <- a + 2
x
